  ...
  <div class="anniversary-list">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <el-input v-model="searchKeyword" placeholder="搜索名称..." clearable prefix-icon="Search" style="width: 220px" @input="loadList" />
        <el-select v-model="filterTag" placeholder="标签筛选" clearable style="width: 150px" @change="loadList">
      ...
      <el-button type="primary" @click="openCreate"><el-icon><Plus /></el-icon>新建纪念日</el-button>
    </div>
    ...
    <div v-loading="loading" class="list-body">
      ...
      <div v-else class="anniversary-cards">
        <div v-for="item in list" :key="item.id" class="anniversary-card" :class="countdownClass(item.daysUntil)" @click="openDetail(item)">
          <div class="card-left">
          ...
          <div class="card-body">
            ...
            <div class="card-meta">
              <el-tag v-if="item.repeatType !== 'NONE'" size="small" type="info">{{ repeatText(item.repeatType) }}</el-tag>
              <el-tag v-if="item.tags" v-for="tag in parseTags(item.tags)" :key="tag" size="small" type="success" class="tag-chip">{{ tag }}</el-tag>
            </div>
          </div>
    ...
    <!-- 新建/编辑对话框 -->
    <el-dialog v-model="showForm" :title="editingItem ? '编辑纪念日' : '新建纪念日'" width="520px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item v-if="form.remindEnabled" label="提醒时间">
          <el-time-picker v-model="form.remindTime" placeholder="选择时间" format="HH:mm" value-format="HH:mm" />
        </el-form-item>
      ...
      <template v-if="detailItem">
        <div class="detail-header">
          <div class="detail-countdown" :class="countdownClass(detailItem.daysUntil)">
            ...
            <template v-else-if="detailItem.daysUntil > 0">还有 {{ detailItem.daysUntil }} 天</template>
            <template v-else>已过去 {{ Math.abs(detailItem.daysUntil) }} 天</template>
          </div>
          <h2>{{ detailItem.name }}</h2>
        </div>
        <el-descriptions :column="1" border>
          <el-descriptions-item label="日期">{{ formatDate(detailItem.date) }}</el-descriptions-item>
          <el-descriptions-item label="下一次发生日">{{ formatDate(detailItem.nextDate) }}</el-descriptions-item>
          <el-descriptions-item label="重复类型">{{ repeatText(detailItem.repeatType) }}</el-descriptions-item>
          <el-descriptions-item label="提醒">{{ detailItem.remindEnabled ? '已开启' : '未开启' }}</el-descriptions-item>
          <el-descriptions-item v-if="detailItem.nextRemindTimes && detailItem.nextRemindTimes.length > 0" label="提醒时间">
...
const form = reactive({
  name: '', date: '', repeatType: 'NONE', remindEnabled: false,
  remindTime: '09:00', tags: '', notes: ''
})
...
const repeatText = (t: string) => ({ NONE: '不重复', YEARLY: '每年', MONTHLY: '每月', WEEKLY: '每周' } as any)[t] || t
...
const openCreate = () => {
  ...
  Object.assign(form, { name: '', date: '', repeatType: 'NONE', remindEnabled: false, remindTime: '09:00', tags: '', notes: '' })
...
const editFromDetail = () => {
  ...
  remindDaysArr.value = detailItem.value.remindDaysBefore ? detailItem.value.remindDaysBefore.split(',').map((s: string) => s.trim()) : ['0']
...
<style scoped>
.anniversary-list { height: 100%; display: flex; flex-direction: column; }
.toolbar { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; gap: 12px; flex-wrap: wrap; }
.toolbar-left { display: flex; align-items: center; gap: 10px; }
.list-body { flex: 1; overflow-y: auto; }
.anniversary-cards { display: flex; flex-direction: column; gap: 10px; }
.anniversary-card { display: flex; align-items: center; padding: 16px; background: #fff; border-radius: 8px; border: 1px solid #e8e8e8; cursor: pointer; transition: all 0.2s; }
.anniversary-card:hover { box-shadow: 0 2px 12px rgba(0,0,0,0.08); transform: translateY(-1px); }
.card-left { width: 36px; flex-shrink: 0; }
.card-body { flex: 1; min-width: 0; }
.card-name { font-size: 16px; font-weight: 600; color: #303133; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.card-date { font-size: 13px; color: #909399; margin-top: 4px; }
...
.countdown-today { color: #F56C6C; }
.countdown-urgent { color: #F56C6C; }
.countdown-near { color: #E6A23C; }
.countdown-far { color: #909399; }
.next-date { font-size: 12px; color: #c0c4cc; margin-top: 4px; }
.tag-chip { margin-right: 0; }
.detail-header { text-align: center; margin-bottom: 20px; }
.detail-countdown { font-size: 18px; font-weight: 600; margin-bottom: 8px; }
.detail-countdown.countdown-today { color: #F56C6C; }
.detail-countdown.countdown-urgent { color: #F56C6C; }
.detail-countdown.countdown-near { color: #E6A23C; }
.detail-header h2 { margin: 0; font-size: 22px; }
.detail-actions { display: flex; gap: 10px; margin-top: 20px; justify-content: center; }
.remind-icon { font-size: 16px; }
</style>
