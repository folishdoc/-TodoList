...
const isOverdue = (task: any) =>
  task.dueDate && task.status !== 1 && new Date(task.dueDate) < new Date()
...
const getBarStyle = (task: any) => {
  ...
  let si = findDayIndexInBar(sd || dd!),
    ei = findDayIndexInBar(dd || sd!)
  if (si === -1 && ei === -1) return { display: 'none' }
  if (si === -1) si = 0
  if (ei === -1) ei = barDays.value.length - 1
...
const onBarPointerDown = (e: PointerEvent, task: any) => {
  ...
  const si = findDayIndexInBar(sd),
    ei = findDayIndexInBar(dd)
  dragState.value = {
    task,
    mode: 'move',
    startX: e.clientX,
