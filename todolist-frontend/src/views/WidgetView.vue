...
<template>
  <div class="widget" :class="{ 'widget-light-theme': settings.theme === 'light' }">
    <!-- 完整编辑弹窗 -->
    <el-dialog v-model="showEditDialog" title="编辑任务" width="95%" :close-on-click-modal="false" destroy-on-close
    ...
    >
      <div :class="settings.theme === 'light' ? 'widget-light-theme' : ''" style="--bg: #1a1a2e">
        <TaskEditPanel v-if="editingTaskId" :task="getEditingTask()" mode="dialog" @close="showEditDialog = false" @changed="onEditChanged" />
      </div>
    </el-dialog>
    ...
    <div class="widget-header" data-tauri-drag-region>
      ...
      <div class="header-actions" style="-webkit-app-region: no-drag">
        ...
        <button class="header-btn header-close" @click="handleClose" title="关闭">
          <svg width="8" height="8" viewBox="0 0 8 8"><line x1="0.5" y1="0.5" x2="7.5" y2="7.5" stroke="currentColor" stroke-width="1.2"/><line x1="7.5" y1="0.5" x2="0.5" y2="7.5" stroke="currentColor" stroke-width="1.2"/></svg>
        </button>
      </div>
    </div>
    ...
    <div v-if="showFilterMenu" class="filter-menu" @click.stop>
      <div class="filter-item" :class="{ active: currentFilter === 'all' }" @click="selectFilter('all')">全部任务</div>
      <div class="filter-item" :class="{ active: currentFilter === 'today' }" @click="selectFilter('today')">今日任务</div>
      <div class="filter-item" :class="{ active: currentFilter === 'upcoming' }" @click="selectFilter('upcoming')">未来任务</div>
    ...
    <!-- 任务列表 -->
    <div class="widget-body" @click="showFilterMenu = false; showSettings = false">
        ...
        >
          ...
          <div class="task-content" @click="handleEdit(task)">
            ...
            <div class="task-meta">
              ...
              <span v-if="task.repeatRule" class="tag-repeat">🔄 {{ getRepeatLabel(task.repeatRule) }}</span>
...
.widget-dialog-dark { background: #1a1a2e !important; border: 1px solid rgba(255,255,255,0.1) !important; }
.widget-dialog-dark .el-dialog__header { background: #1a1a2e; border-bottom: 1px solid rgba(255,255,255,0.06); }
.widget-dialog-dark .el-dialog__title { color: #e0e0e0; font-size: 14px; }
.widget-dialog-dark .el-dialog__body { background: #1a1a2e; color: #e0e0e0; }
.widget-dialog-dark .el-dialog__headerbtn .el-dialog__close { color: #888; }
.widget-dialog-dark .el-dialog__headerbtn:hover .el-dialog__close { color: #e0e0e0; }
.widget-dialog-dark .el-input__inner,
.widget-dialog-dark .el-textarea__inner { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.1); color: #e0e0e0; }
.widget-dialog-dark .el-input__inner:focus,
.widget-dialog-dark .el-textarea__inner:focus { border-color: #667eea; }
.widget-dialog-dark .el-tag--default,
.widget-dialog-dark .el-tag--info { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.1); color: #e0e0e0; }
.widget-dialog-dark .el-button--default { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.1); color: #e0e0e0; }
.widget-dialog-dark .el-button--default:hover { background: rgba(255,255,255,0.12); }
.widget-dialog-dark .el-divider__text { background: #1a1a2e; color: #888; }
.widget-dialog-dark .el-checkbox__label { color: #e0e0e0; }

.widget-dialog-light { background: #f5f5f5 !important; border: 1px solid rgba(0,0,0,0.08) !important; }
.widget-dialog-light .el-dialog__header { background: #f5f5f5; border-bottom: 1px solid rgba(0,0,0,0.06); }
.widget-dialog-light .el-dialog__title { color: #303133; font-size: 14px; }
.widget-dialog-light .el-dialog__body { background: #f5f5f5; color: #303133; }
.widget-dialog-light .el-dialog__headerbtn .el-dialog__close { color: #909399; }
.widget-dialog-light .el-dialog__headerbtn:hover .el-dialog__close { color: #303133; }
</style>
...
.filter-dropdown:hover { background: var(--hover); color: var(--text); }
.filter-arrow { font-size: 10px; opacity: 0.5; }
...
.header-close:hover { background: #e81123; color: #fff; }
.header-close:active { transform: scale(0.92); }
.gear-icon { font-size: 12px; line-height: 1; }
...
.filter-item:active { background: rgba(102,126,234,0.15); }
.filter-item.active { color: #667eea; font-weight: 600; }
.filter-item.dim { color: var(--text-secondary); cursor: default; }
...
.theme-options button:hover:not(.active) { background: var(--hover); }
.theme-options button:active { transform: scale(0.96); }
...
.task-check.pri-high { color: #f56c6c; }
.task-check.pri-low { color: var(--text-secondary); }
.task-item.done .task-check { color: #67c23a; }
...
.tag-pri.high { background: rgba(245,108,108,0.2); color: #f56c6c; }
.tag-pri.mid { background: rgba(230,162,60,0.2); color: #e6a23c; }
.tag-pri.low { background: rgba(144,147,153,0.2); color: #909399; }
...
.task-del:hover { opacity: 1 !important; color: #f56c6c; background: var(--btn-hover); }
...
.widget-input::placeholder { color: var(--text-secondary); }
.widget-input:focus { background: var(--btn-hover); }
